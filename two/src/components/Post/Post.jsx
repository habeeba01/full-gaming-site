...
import { commentPost, deletePost, likePost,reportPost } from "../../API";
...
const Post = ({ singlepost, post }) => {
    ...
    const isReported = post?.report?.includes(id);
    ...
    const reportHandler = async () => {
        const data = await customFetch(reportPost, post._id, token, !isReported);
        if (data) {
            if (singlepost) {
                dispatch(setSinglePost(data.posts));
            } else {
